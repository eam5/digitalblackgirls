...
if ($hasVisualMedia) {
    ...
    queue_js_string('
        jQuery(document).ready(function() {
            DigitalBlackGirls.useLightslider(' . count($visualMedia) . ');
        });
    ');
}
...
<?php if ($hasVisualMedia): ?>
	...
	<?php $squareThumbnail = digitalblackgirls_get_square_thumbnail_url($mediaFile, $this); ?>
        ...
        <?php else: ?>
            ...
            >
                <div style="display: none;" id="video-<?php echo $visualMediaCount; ?>">
                    <?php $tracksPresent = digitalblackgirls_check_for_tracks($otherFiles); ?>
                    <video class="lg-video-object lg-html5" controls preload="none" <?php echo ($tracksPresent) ? 'crossorigin="anonymous"' : ''; ?>>
                        ...
                        <?php foreach ($otherFiles as $key => $otherFile): ?>
                            <?php if ($otherFile->original_filename == "$mediaName.vtt"): ?>
                                <?php echo digitalblackgirls_output_text_track_file($otherFile); ?>
